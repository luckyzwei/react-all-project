        ...
        AuthProvider.getAccessToken().then((reslove,reject)=> {
            ...
            if(resData.resultCode=='100'){
                ...
                groupList = groupList.map(v => ({
                    ...v,
                    enterGroupStatus: v.id==groupId?2:v.enterGroupStatus
                }))
    ...
    render(){
        ...
        const columns = [{
            ...
            render: (text, record) => {
                ...
                ?<span className="operate" onClick={()=>{this.stopEnterGroup(record.id)}}>停止入群</span>
                :'-'
            }
        }]
        return (
            <div className="fast-edit-build-group">
                ...
                <div className="bottom">
                    <span className='backBtn' onClick={()=>{selectType('SELECT')}}>返回</span>
                </div>
                {
