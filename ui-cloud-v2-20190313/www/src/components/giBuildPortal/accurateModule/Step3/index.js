...
import './index.css'
import '../../manualModule/Step3/index.css'
...
export default class InGroupRule extends Component {
    ...
    verifyHandle = (ruleData) => {
        ...
        // 日期判断
        // let EXPECTED_DATE = params.gmJgRuleItems.find(v => v.optionCode=='EXPECTED_DATE')
        // if(EXPECTED_DATE&&EXPECTED_DATE.isMandatory==1&&(EXPECTED_DATE.values[0].startValue==''||EXPECTED_DATE.values[0].endValue=='')){
        //     sendEvent('message', {txt: '请选择日期',code: 1004})
        //     return false
        // }else if(EXPECTED_DATE&&new Date(EXPECTED_DATE.values[0].startValue).getTime()>new Date(EXPECTED_DATE.values[0].endValue).getTime()){
        //     sendEvent('message', {txt: '日期格式错误',code: 1004})
        //     return false
        // }
        // // 非必填状态判断
        // if(EXPECTED_DATE&&EXPECTED_DATE.isMandatory!=1&&((EXPECTED_DATE.values[0].startValue==''&&EXPECTED_DATE.values[0].endValue!='')||(EXPECTED_DATE.values[0].startValue!=''&&EXPECTED_DATE.values[0].endValue==''))){
        //     sendEvent('message', {txt: '请选择日期',code: 1004})
        //     return false
        // }

        // // 日期判断
        // let CUSTOM_DATE = params.gmJgRuleItems.find(v => v.optionCode=='CUSTOM_DATE')
        // if(CUSTOM_DATE&&CUSTOM_DATE.isMandatory==1&&(CUSTOM_DATE.values[0].startValue==''||CUSTOM_DATE.values[0].endValue=='')){
        //     sendEvent('message', {txt: '请选择日期',code: 1004})
        //     return false
        // }else if(CUSTOM_DATE&&new Date(CUSTOM_DATE.values[0].startValue).getTime()>new Date(CUSTOM_DATE.values[0].endValue).getTime()){
        //     sendEvent('message', {txt: '日期格式错误',code: 1004})
        //     return false
        // }
        // // 非必填状态判断
        // if(CUSTOM_DATE&&CUSTOM_DATE.isMandatory!=1&&((CUSTOM_DATE.values[0].startValue==''&&CUSTOM_DATE.values[0].endValue!='')||(CUSTOM_DATE.values[0].startValue!=''&&CUSTOM_DATE.values[0].endValue==''))){
        //     sendEvent('message', {txt: '请选择日期',code: 1004})
        //     return false
        // }

        // // 城市判断
        // let CITY = params.gmJgRuleItems.find(v => v.optionCode=='CITY')
        // if(CITY&&CITY.isMandatory==1&&CITY.values.length==0){
        //     sendEvent('message', {txt: '请选择城市',code: 1004})
        //     return false
        // }

        // // STORE
        // let STORE = params.gmJgRuleItems.find(v => v.optionCode=='STORE')
        // if(STORE&&STORE.isMandatory==1&&STORE.values.length==0){
        //     sendEvent('message', {txt: '请选择门店',code: 1004})
        //     return false
        // }

        // // 班级判断
        // let CLASS = params.gmJgRuleItems.find(v => v.optionCode=='CLASS')
        // if(CLASS&&CLASS.isMandatory==1&&CLASS.values.length==0){
        //     sendEvent('message', {txt: '请选择班级',code: 1004})
        //     return false
        // }

        // // 入群渠道判断
        // let CHANNEL = params.gmJgRuleItems.find(v => v.optionCode=='CHANNEL')
        // if(CHANNEL&&CHANNEL.isMandatory==1&&CHANNEL.values.length==0){
        //     sendEvent('message', {txt: '请选择渠道',code: 1004})
        //     return false
        // }

        // // 推荐码判断
        // let RECOMMENDATION_CODE = params.gmJgRuleItems.find(v => v.optionCode=='RECOMMENDATION_CODE')
        // if(RECOMMENDATION_CODE&&RECOMMENDATION_CODE.isMandatory==1&&RECOMMENDATION_CODE.values[0].displayValue==''){
        //     sendEvent('message', {txt: '请输入推荐码',code: 1004})
        //     return false
        // }
        // // 自定义选择框
        // let CUSTOM_SELECT = params.gmJgRuleItems.find(v => v.optionCode=='CUSTOM_SELECT')
        // if(CUSTOM_SELECT&&CUSTOM_SELECT.isMandatory==1&&CUSTOM_SELECT.values.length==0){
        //     sendEvent('message', {txt: '请选择自定义下拉框选项',code: 1004})
        //     return false
        // }
        // // 自定义输入框
        // let CUSTOM_IPT = params.gmJgRuleItems.find(v => v.optionCode=='CUSTOM_INPUT')
        // if(CUSTOM_IPT&&CUSTOM_IPT.isMandatory==1&&CUSTOM_IPT.values[0].displayValue==''){
        //     sendEvent('message', {txt: '请输入自定义输入框选项',code: 1004})
        //     return false
        // }
        // return true
    ...
    submitPageParams =()=> {
        ...
        sendEvent('message', {txt: "正在创建群，请稍等~",code: 1000})
        let url = `${API_PATH}/groupadmin-api/authsec/robothost/create/batch/group?taskName=${templateData.name}`
        AuthProvider.getAccessToken().then((reslove,reject)=> {
            return promiseXHR(url ,{type:'Bearer',value:reslove},ruleData,'post')
        }).then(res => {
            const resData = JSON.parse(res)
            if(resData.resultCode=='100'){
                this.setState({
                ...
                this.props.setparamsHandle('joinGroupRuleId','',true,this.state.ruleData.filter(v => !v.groupRule.id))
                sendEvent('message', {txt: "规则提交成功",code: 1000})
            }else if(resData.resultCode=='03801414'||resData.resultCode=='03801412'){
                this.setState({
                    submiting: false
                })
                let newRuleData = this.state.ruleData.filter(v => !v.groupRule.id).map((v,i) => ({
                    ...v,
                    groupName: resData.resultContent[i].groupName,
                    groupRule: resData.resultContent[i].groupRule,
                    errorFlag: resData.resultContent[i].errorFlag
                }))
                this.props.setparamsHandle('joinGroupRuleId','',false,newRuleData)
            ...
            }else if(resData.resultCode=='03801412'){
                throw {
                    txt:'规则相同冲突',
                    type: 'self'
                }
            ...
            this.props.setparamsHandle('joinGroupRuleId','',false,this.state.ruleData.filter(v => !v.groupRule.id))
    ...
    cleanRuleData = () => {
        ...
        let ruleData = this.state.ruleData.filter(v => !v.groupRule.id).map(v => {
    ...
    render() {
        let {cancelFlag,ruleData,ruleTemp,dataCache,modalStyle} = this.state;
        let {templateData,viewType,nextStep,ruleType} = this.props;
        // console.log(ruleData)
        return (
            ...
            <div className="inner">
                ...
                <div className="InGroupRuleRow pageSize">
                    ...
                    <BuildRule ruleData={ruleData} updateRules={this.updateRules} updateRangeRules={this.updateRangeRules} dataCache={dataCache} dataCacheHandle={this.dataCacheHandle}/>
                    {/* {
                        ruleData.length>0
                        ?ruleData.map((v,i)=>{
                            return v.selected?<BuildRule key={i} groupRuleData={v} updateRules={this.updateRules} updateRangeRules={this.updateRangeRules}/>:null
                        })
                ...
                <ModalBox 
                    ...
                    modalStyle={{width:'430px',...modalStyle}}//修改样式，默认最小高度220px，宽度420px
                    closeModalFunc={this.closeMadal} //关闭弹出框函数
