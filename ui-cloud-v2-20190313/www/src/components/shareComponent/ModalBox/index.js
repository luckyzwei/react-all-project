...
export default class ModalBox extends Component {
    ...
    confirmModal(e) {
        ...
        modalBoxContent.addEventListener('transitionend',this.props.confirmFunc)
    ...
    closeModal(e) {
        ...
        const modalBox = document.querySelectorAll('.ModalBox-animate')[0]
        const modalBoxContent = document.querySelectorAll('.ModalBoxContent-animate')[0]
        modalBoxContent.addEventListener('transitionend',this.props.closeModalFunc)
        modalBox.style.opacity = 0
        modalBoxContent.style.cssText = `top: ${modalStyle.top};left: ${modalStyle.left};transform: translate(-50%,-50%) scale(0);`
    }
    ...
    shouldComponentUpdate(nextProps,nextState){
        if(this.props.modalStatus&&!nextProps.modalStatus){
            const modalBoxContent = document.querySelectorAll('.ModalBoxContent-animate')[0]
            if(modalBoxContent){
                modalBoxContent.removeEventListener('transitionend',this.props.confirmFunc)
                modalBoxContent.removeEventListener('transitionend',this.props.closeModalFunc)
            }
        }
        return true
    }
