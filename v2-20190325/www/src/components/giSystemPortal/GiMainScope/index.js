...
export default class GiMainScope extends Component {
    ...
    componentDidMount(){
        ...
        this.requestGroupList(pageInfo,{}).then(res => {
            ...
            if(data.resultCode=='100'){
                this.setState({
                    data: data.resultContent.map((v,i) => {
                        return {
                            ...v,
                            select: false,
                            seqNo: i+1
                ...
                this.setState({
                    data: data.resultContent.map((v,i) => {
                        return {
                            ...v,
                            select: false,
                            seqNo: pageInfo.currentPage*pageInfo.pageSize+1+i
                        }
                    }),
                    pageInfo: data.pageInfo?data.pageInfo:{
                        currentPage: 0,
    ...
    render() {
        let {isBatch,editFlag,groupId,groupIds,data,pageInfo,checkAll,guideFlag,loading} = this.state;
        ...
        return (
            <div className='gi-container' style={{backgroundColor:'#F8F8F9'}}>
                <SearchBox 
                ...
                <GroupTable 
                    data={data} 
                    goEditHandle={this.goEditHandle} 
                    actions={actions} 
                    pageInfo={pageInfo} 
                    handlersearchData={this.handlersearchData}
                    loading={loading}
                    guideFlag={guideFlag}
                    tableBatchHandle={this.tableBatchHandle}
