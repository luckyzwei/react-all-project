...
export const PublicDom = ({item, ImgH}) => {
    return (
        <div className="Dom-item">
            <span className='Dom-item-icon'></span>
            <div className="public-box">
                <div className="item-content-box">
                    <img src={item.uri} className="item-icon"/>
                    <p>{item.title}</p>
                </div>
...
export const VoiceDom = ({item, ImgH}) => {
    return (
        <div className="Dom-item voice-box">
            <span className='Dom-item-icon'></span>
            <div className="item-content-box">
                <em></em>{item.files[0].type}"
            </div>
        </div>
    )
}
...
export const AppModule = ({item}) => {
    return (

        <div className="Dom-item AppModule">
            <span className='Dom-item-icon'></span>
            <div className='Dom-item-AppModule'>
                <div className="Dom-item-content-trg"></div>
                <div className='appModule'>
                    ...
                    <div className='appSrc'>
                        {item.files.find(item=>item.fileType ==='image')?
                            <img
                                src={item.files[0].filePath?item.files.find(item=>item.fileType ==='image').filePath: process.env.PUBLIC_URL+'/images/icon/default.png'}
                            />
                            : ''
                        }
                    </div>
                    <div className='appFooter'>
                        <i className='wxlogo'></i>
                        <span>小程序</span>
                    </div>
                </div>
            </div>
