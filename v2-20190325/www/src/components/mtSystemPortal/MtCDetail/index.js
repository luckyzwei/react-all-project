...
export default class MtCDetail extends Component {
    ...
    changeNav(e) {
        ...
        if (parseInt(e.target.getAttribute('value')) === 3) {
            for (let i = 0; i < lists.length; i++) {
                if (lists[i].status == 1) {
                    list.push(lists[i])
                }
        ...
        if (parseInt(e.target.getAttribute('value')) === 1) {
            for (let i = 0; i < lists.length; i++) {
                if (lists[i].status != 2 && lists[i].status != 5 && lists[i].status != 1) {
                    list.push(lists[i])
                }
            }
        }
    ...
    componentDidMount() {
        ...
        getGroupTask(data).then(res => {
            ...
            if (res.resultCode === '100') {
                ...
                let arr = []
                res.resultContent.excBatchInfos[0].excBatchDetail.forEach(v1 => {
                    arr = []
                    v1.status != 5&&arr.push(v1)
                    v1.sendRecords&&v1.sendRecords.forEach(v => {
                        !(arr.find(item => {
                            return item.resultDetail == v.resultDetail
                        }))&&arr.push(v)
                    })
                    v1.sendRecords = arr
                })
                this.setState({
                    ...
                    sendList: res.resultContent.excBatchInfos[0].excBatchDetail.filter((item) => item.status==1),
                    otherList: res.resultContent.excBatchInfos[0].excBatchDetail.filter((item) => item.status!==5&&item.status!==2&&item.status!==1)
                }, () => {
                    if (wexx) {
                        // console.log(wexx);
    ...
    render() {
        let {list,lists, successList,sendList, failList, otherList, paramsValue, paramsData, sendRecords, changeValue} = this.state;
                    ...
                    <div className="MtCDetail-content-box">
                        ...
                        <div className="MtCDetail-content-box-right">
                            ...
                            <div className="MtCDetail-content-box-right-nav" onClick={this.changeNav}>
                                ...
                                <span
                                    className={changeValue === 3 ? "MtCDetail-content-box-right-nav-item active" : "MtCDetail-content-box-right-nav-item"}
                                    value='3'>发送中({sendList.length})</span>
                                <span
                                    ...
                                    list.map((item, index) => {
                                        return (
                                            <aside className="list-item" key={index}>
                                                ...
                                                {
                                                    item.status != 2 && item.status != 5 && item.status != 1 ? <span
                                                        className="list-item-status list-item-status-un">未发送</span> : ''
                                                }
                                                {
                                                    item.status == 1 ? <span
                                                        className="list-item-status list-item-status-an">发送中</span> : ''
                                                }
                                                {
                                                    item.status == 2 ? <span
