...
const component = (item,updateRules,updateRangeRules,allItems,dataCache,dataCacheHandle,currentIndex,disabled,ruleDataLength) => {
    ...
    switch (item.optionCode) {
        ...
        case 'STORE':
            ...
            console.log(item.optionCode+currentIndex+ruleDataLength)
...
export default class BuildRule extends Component {
    ...
    componentDidMount(){
        if(this.state.ruleData.length>0){
            ...
            let currentIndex = this.state.ruleData.findIndex(v => v.selected)
            if(currentIndex!=-1){
                // 切换默认值
                let hasStore = this.state.ruleData[currentIndex].groupRule.gmJgRuleItems.find(v => v.optionCode=='STORE')?true:false
                let selectItem
                if(hasStore){
                    // 如果有门店，选择非城市第一个
                    selectItem = this.state.ruleData[currentIndex].groupRule.gmJgRuleItems.find(v => v.optionCode!='CITY')
                }else{
                    // 如果没有门店，选择第一个
                    selectItem = this.state.ruleData[currentIndex].groupRule.gmJgRuleItems[0]
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if((prevState.ruleData.length==nextProps.ruleData.length)||(prevState.ruleData.length==0&&nextProps.ruleData.length>0)){
            // 如果群长度没有发生变化，并且选中的群位置发生改变，说明需要把规则tab置为0
            let currentIndex = prevState.ruleData.findIndex(v => v.selected)
            let nextIndex = nextProps.ruleData.findIndex(v => v.selected)
            console.log(currentIndex,nextIndex)
            if((currentIndex!=nextIndex)||(currentIndex!=-1&&!prevState.selectItem.optionCode)){
                // 切换默认值
        ...
        }else if(prevState.ruleData.length!=nextProps.ruleData.length){
            console.log('****')
            // 如果长度发送变化，currentIndex需要更新
            let nextIndex = nextProps.ruleData.findIndex(v => v.selected)
            return {
                currentIndex: nextIndex,
                ruleData: nextProps.ruleData
    ...
    render(){
        const {tabIndex,currentIndex,selectItem,ruleData} = this.state
        const {updateRules,updateRangeRules,dataCache,dataCacheHandle} = this.props
