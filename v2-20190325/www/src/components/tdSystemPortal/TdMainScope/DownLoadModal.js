...
import '../../homeSystemPortal/DownLoadModal/index.css'
...
export default class Index extends Component{
    ...
    exportAsExcel = () => {
        ...
        let tenantId = this.props.userInfo.info.userinfo.tenantId;

        let { dateArr } = self.state;
        let url = API_PATH + '/message-measure/authsec/downloadLaunchClickReport/' + tenantId;
        if ( dateArr == '') {
            sendEvent("message", { txt: "请选择时间段", code: 1003 })
            return;
        }
        this.setState({ limitLoad: true })
        tongji('Lzc_TouFangShuJu_DaoChuShuJu')
        AuthProvider.getAccessToken().then(resolve => {
            return promiseXHR(url, { type: 'Bearer', value: resolve }, { dateStart: dateArr[0], dateEnd: dateArr[1]}, 'POST')
        }).then(response => {
