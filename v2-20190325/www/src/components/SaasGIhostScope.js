...
import GIhostMain from './giHostPortal/GIhostMain'
...
export default class SassGIeditScope extends Component {
  ...
  componentDidMount(){
    document.title = '微信托管 | 群管理 | 栗子云'
  }
  componentWillMount() {
    const {naviMetaData,actions} = this.props
    if(naviMetaData.flatNaviList!==null){
      const isUse = naviMetaData.flatNaviList.find(v => v.code=='HostScope')
      if(!isUse){
        actions.goTo('/v2/authority?scope=HostScope')
